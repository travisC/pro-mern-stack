...
/***/ (function(module, exports, __webpack_require__) {
	...
	app.get('/api/issues', (req, res) => {
	  ...
	  if (req.query._summary === undefined) {
	    ...
	    db.collection('issues').aggregate([{ $match: filter }, { $group: { _id: { owner: '$owner', status: '$status' }, count: { $sum: 1 } } }]).toArray().then(results => {}).catch(error => {
	      console.log(error);
	      res.status(500).json({ message: `Internal Server Error: ${error}` });
	    });
	  }
...
//# sourceMappingURL=0.16a853206a3f90dd2f61.hot-update.js.map